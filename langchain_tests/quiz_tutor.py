...
class MarkdownTutor:
    ...
    def generate_quiz(self, num_questions=5, multiple_choice=True):
        """Generate a quiz from the lesson content"""
        quiz_prompt = f"""
        ...
        docs = self.retriever.get_relevant_documents(
            "HTML basics and semantic HTML concepts"
        )
        ...
        try:
            ...
            # Try to recover from badly formatted JSON
...
if __name__ == "__main__":
    ...
    print("Type 'quiz' to generate a quiz, or ask questions about the lesson.\n")
    ...
    while True:
        ...
        if user_input.startswith("quiz"):
            ...
            num_q = int(parts[1]) if len(parts) > 1 else 5
            print("🧠 Generating quiz...")
            tutor.generate_quiz(num_questions=num_q, multiple_choice=True)
            print("\n" + tutor.ask_quiz_question())
            continue

        if tutor.quiz["questions"]:
            print(tutor.answer_quiz(user_input))
            q_text = tutor.ask_quiz_question()
            print(q_text)
            continue
